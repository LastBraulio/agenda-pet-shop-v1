...
Class ModelCubiculos extends Conexiones_local{
    ...
    public function mostrar_cubiculos_ocupados($ids)
	{	
		$sql = " SELECT fc.idfpos_cubiculo,fc.name,if(fc.status=1,'<button type=\"\button\"\ class=\"\btn btn-dark\"\>OCUPADO</button>
        ','<button type=\"\button\"\ class=\"\btn btn-warning\"\>Warning</button>') AS ESTADO FROM fpos_cubiculo fc
         WHERE fc.status IN(1,2) AND fc.id_bussines=".$ids.";";
